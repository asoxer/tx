...
jQuery(document).ready(function(){
    ...
    $('#order-standard_cart .group-billing-cycle input:radio').on('ifChecked', function(event) {
    ...
    jQuery("#btnNewUserSignup").click(function() {
        jQuery("#containerExistingUserSignin").slideUp('', function() {
            jQuery("#containerNewUserSignup").hide().removeClass('hidden').slideDown('', function() {
                ...
                jQuery("#containerNewUserSecurity").removeClass('hidden').show();
    ...
    if(jQuery('#products').length > 0) {
        (new Image()).src = './cart.php?a=view';
    }

    jQuery("#btnCompleteOrder").click(function (e) {
        var isloading = jQuery(this).attr('data-loading') === '1';
        if(isloading) {
            alert('订单提交中，请不要重复提交');
            return false;
        } else {
